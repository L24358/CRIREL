...
def firing_r_output (filename, output_filename):
    l = ['OutControl', 'FileName:' + str(output_filename),\
         'Type=FiringRate','FiringRateWindow=50','PrintStep=5',\
...
def stimulus(filename, key, l, trial_t):
    ...
    if cmd=='ramp':
        ...
        external_stimulus(filename, key[1], key[0], l[2], l[3])
        external_stimulus(filename, key[1], key[0]+float(l[1]), l[2], 0)
    elif cmd=='sine':
        nums = trial_t/l[3]
        x = np.arange(nums)
...
def exec_pro(**kwargs):
    ...
    kw = {'filename':'network.pro', 'ID':2447, 'event_dic':{},\
          ...
          'baseline':False, 'MemPot':'MemPot.dat'}
    kw.update(kwargs)
    ...
    Frate, Mot, baseline, MemPot = kw['Frate'], kw['Mot'], kw['baseline'], kw['MemPot']
