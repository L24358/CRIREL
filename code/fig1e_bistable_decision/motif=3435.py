...
This is the same code as Fig_1_new_3435.py, but with the parameters setup so that it runs for motif A instead of B.
==========================================='''
...
N = 2
EElist = list(range(0,205,5))
IElist = [20]
IIlist = list(range(0,125,5))
duration, amp, noise = 100, 5, 0
...
def def_decision2(FRs, mot, tpe):
	...
	for key in sorted(dic.keys()):
		...
		if (res1!='neither') and (res2!='neither') and res1!=res2: return True #print(key) to check
	return False
...
def heatmap(ID, tpe, small, xaxis, yaxis, outputfigname, motherpath=os.getcwd()):
	...
	for y in yaxis:
		xlist = []
		for x in xaxis:
			b2 = clss.branch('EE='+str(y)+'_IE='+str(20)+'_II='+str(x), b1.pathlink) 
			b4 = clss.branch('EE='+str(y)+'_IE='+str(20)+'_II='+str(x), b3.pathlink) 
		...
		ylist.append(xlist)
	#plot
	myColors = ('whitesmoke','royalblue','tomato','mediumorchid')
	cmap = LinearSegmentedColormap.from_list('Custom', myColors, len(myColors))
	ax = sns.heatmap(ylist, xticklabels=xaxis, yticklabels=yaxis, cmap=cmap)
	new_xaxis = list(np.arange(0,120+5,20))
	new_yaxis = list(np.arange(0,200+5,20))
	new_yaxis.reverse()
	plt.xticks(np.arange(.5,24.5+1,4), new_xaxis)
	plt.yticks(np.arange(.5,40.5+3,4), new_yaxis)
...
if __name__ == '__main__':
    #=====Makes files to run=====#
    if True:
        decision(clss.motif(3435), 0.8)
        bistable(clss.motif(3435))
    #====Analyzes the results=====#
    if False:
        heatmap(3435, 'inh', 0.8, IIlist, EElist, '3435_small=0.8_pass='+str(small2perc[0.8])+'.png')
